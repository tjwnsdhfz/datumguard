...
- 작성 기준일: 2026-07-13
- 현재 공개 release: `v0.4.0`
- 기준 source: `0f9e0b08210aa6bc4fc8cc6bb1d398292b12e832`
...
- `v0.4.0` public release, Vercel web, Render API, exact-SHA deployment smoke
- 413 pytest, 41 Playwright, container/SBOM/security gate
...
| GitHub community | community health 100% | contribution, conduct, issue form 공개 완료 |
| GitHub Discussions | 활성 | 소프트 런치와 재현 가능한 edge-case 수집 경로 확보 |
| GitHub social preview | custom 1280×640 proof image 적용 | release asset과 동일한 deterministic image |
| GitHub profile | CAD assurance bio, Case Study URL, hireable 공개 | name/location/pinned/profile README는 사용자 선택으로 남김 |
...
- [x] GitHub custom social preview, profile, contribution path
...
1. GitHub growth foundation, community health 100%, Discussions, contribution path
2. web launch readiness, Case Study, SEO/structured data와 privacy-aligned first-party attribution
3. 실제 Rhino 8 + Grasshopper + Cordyceps round-trip과 GUID/hash evidence
4. external DXF completeness matrix와 fail-closed comparison gate
5. v0.4.0 source, CI/security/SBOM, Vercel·Render exact-SHA deployment, strict smoke
6. GitHub profile, custom social preview, release asset, soft-launch copy와 60초 demo script

다음 우선순위:

1. GitHub Discussion 소프트 런치에서 재현 가능한 Rhino/GH·DXF edge case 수집
2. standalone `.3dm`과 단위 불명/tilted datum/out-of-plane failure evidence 추가
3. 60~75초 무음 자막 영상과 15초 README GIF 제작
4. Search Console/sitemap 소유권, pageview baseline, privacy 문서 일치 확인
5. 무료 범위의 uptime 관측 또는 비용 승인 뒤 외부 monitor/error tracker 연결
6. SEV1 rollback drill과 유료 용량 후보 동시성 시험은 별도 승인·환경에서 실행
7. 위 launch gate가 닫힌 뒤 7일 feedback을 분류하고 Show HN/Product Hunt 진행 여부 결정
