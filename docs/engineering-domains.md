...
| OpenBIM evidence research | `/openbim` | 별도 API, MCP 없음 | IFC4+IDS 정보요구조건, IFC integrity, project AABB clearance, protected revision |
...
OpenBIM Evidence Guard도 official bundle approval과 구분된다. baseline/candidate IFC와 IDS를 별도
worker가 다시 열어 재현 가능한 issue evidence를 만들지만, 합성 학생 연구 범위이며
`research_validation_only=true`, `approval_eligible=false`다. IDS 검사와 DatumGuard project
geometry/revision 규칙을 서로 다른 scope로 보고한다.
