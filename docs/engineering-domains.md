...
2. 분야별 writer가 canonical geometry를 R2013 DXF로 직렬화한다.
3. 별도 verifier가 저장된 DXF byte를 다시 읽어 좌표와 제약을 재측정한다.
4. 모든 required check가 통과한 경우에만 공식 ZIP bundle을 만든다.
5. Web과 MCP는 같은 분야별 application service를 호출한다.

Writer의 Shapely geometry나 사용자 입력값을 실제 artifact 측정값으로 재사용하지 않는다. 각 성공 응답은 서로 다른 `contract_hash`, `artifact_hash`와 `summary_source: independent_serialized_dxf_remeasurement`를 제공한다.
