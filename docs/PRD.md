...
| 문서 버전 | 1.5.0 |
| 작성일 | 2026-07-13 |
| 제품 단계 | v0.4.0 공개 Production MVP |
| 설계·감사 범위 | 건축 평면 + 플랜트/반도체 utility piping + 구조 frame screening + 기계/조선 plate + 제한형 3D solid + 외부 CAD/OpenBIM audit |
| 기준 사용자 | 메이커·공학 학습자 |
| 승인 가능 산출물 | 2D 설계의 검증된 DXF bundle; 제한형 Solid의 검증 STEP bundle은 local/CI 전용 |
| 연관 문서 | [TRD](./TRD.md), [Prompt Design](./prompt-design.md), [Production Handoff](./HANDOFF.md) |

## 1. 제품 요약

DatumGuard는 공학설계 요구사항을 수치 기반 contract로 확정하고, CAD 산출물을 독립적으로 다시 읽어 치수·좌표·공차를 검증한 뒤 합격한 결과만 배포하는 Engineering Design Assurance Harness다. Architecture, Piping, Plate는 serialized DXF를 검증하고, FrameGuard는 exact 2D frame solver와 독립 DXF verifier가 함께 통과할 때만 screening DXF를 제공한다. 제한형 Solid은 local/CI에서 serialized STEP을 별도 OpenCascade process로 재입력한다. Artifact Lab과 OpenBIM preview는 외부 artifact를 변경하지 않고 감사하되 제작 승인을 만들지 않는다.
...
MVP는 일반 사용자가 설치 없이 웹에서 2D 설계와 Artifact Lab을 이용하고, 고급 사용자는 동일한 코어를 MCP 도구로 호출한다. Rhino는 미리보기와 교차검증을 담당하며 공식 합격 판정은 독립 DXF/STEP verifier가 담당한다. 공개 Render Free API에서는 Solid 실행 capability를 fail-closed하고, Solid UI·schema와 local/CI 재현 경로만 제공한다.

공개 demo의 기본 경로는 12×8m 합성 건축 평면으로 정확성 보증 흐름을 보여준다. 사용자는 wall, grid, opening, column과 room seed를 직접 이동하거나 mm 값으로 편집하고, 생성된 건축 DXF를 다시 읽은 검증 evidence가 통과한 뒤에만 공식 bundle을 받을 수 있다. `/piping`은 반도체 utility/플랜트 배관 route를, `/frame`은 Rhino/GH 교환과 2D frame screening을, `/plate`는 기계·조선 plate/panel 설계를 제공한다. `/intake`는 외부 CAD informational audit를, `/openbim`은 합성 IFC/IDS 연구 preview를 제공한다. `/solid`은 세 가지 제한형 STEP family를 설명하지만 hosted 실행은 비활성이다.

### 1.1 v0.4.0 release 상태

- Release 기준점: [`v0.4.0`](https://github.com/tjwnsdhfz/datumguard/releases/tag/v0.4.0) / `0f9e0b08210aa6bc4fc8cc6bb1d398292b12e832`
- Release CI: 413 pytest + 41 Playwright + 15-page web build + container/SBOM/fixed-critical scan pass
- Production smoke: API v0.4.0, exact `release_sha`, Architecture·FrameGuard·Artifact Lab canary와 CORS pass; Solid·OpenBIM execution disabled
- Rhino/GH evidence: 실제 Rhino object GUID 6개, exchange/contract/artifact hash, DXF·verification bundle 공개
- External DXF: versioned completeness matrix와 fail-closed `comparison_complete` gate 공개
- Research smoke: genuine OpenSeesPy 6/6 parity + small PyG training/portable-inference parity pass
- 100개 golden contract + 자연어 50개 benchmark: **계획됨, 미완료**
...
8. 합성 benchmark로 정확성, 재현성과 실패를 공개한다. 이 항목은 v0.2.0 이후 계획이며 아직 완료되지 않았다.
...
- 산업표준·건축법·안전규정 준수 인증
- 임의 형상의 범용 3D 기계 부품·assembly 생성 또는 검증. 지원 범위는 mounting plate, angle bracket, flange의 저장 STEP 기하 일치에 한정한다.
- 임의 형상의 범용 CAD 생성
...
| DG-FR-015 | MCP 도구 | P0 | 하위 호환 설계 도구와 Artifact/Solid/FrameGuard/Rhino 도구, 총 19개가 공통 envelope과 오류코드를 사용한다. |
...
| DG-FR-018 | Benchmark·평가 | P1 | 계획: 100개 golden contract와 자연어 50개 평가를 실행하고 pass/fail report를 공개한다. v0.2.0 기준 미완료다. |
...
| Golden contract 실행 성공률 | 목표 100/100 — benchmark 미실행 |
| 모호한 자연어 숫자 무단 추정 | 0건 |
...
- 계획된 Golden contract 100개: 단위 변환, 패턴, locked conflict, feature overlap, outline 외부 feature, tolerance 경계, 재현성 포함
- 계획된 자연어 평가 50개: 명확한 입력, 모호한 숫자, 단위 누락, 상충 조건, 금지된 추정 포함
- 모든 fixture는 합성 데이터이며 공개 재배포 가능해야 한다.
- 평가 리포트에는 통과율뿐 아니라 실패 contract와 violation을 공개한다.
- v0.4.0의 413 pytest와 41 Playwright는 회귀·통합 test evidence이며, FrameGuard의 90-case 합성 solver dataset도 위 100+50 제품 benchmark를 완료한 것으로 계산하지 않는다.
...
9. **계획, 미완료:** 100개 golden contract와 자연어 50개 평가 결과가 공개된다.
...
15. `/frame`의 정상 preset은 exact solver와 DXF verifier를 통과하고, missing-brace preset은 screening DXF 다운로드를 차단한다.
...
| M8a | Docker와 v0.2.1 공개 release — 완료 |
| M8b | 100개 golden contract + 자연어 50개 benchmark와 결과 공개 — 계획, 미완료 |
| M9 | v0.3.0 FrameGuard screening + public OpenBIM research UI/evidence — 완료 |
| M10 | v0.4.0 Rhino/GH verified round trip + external DXF completeness gate + public evidence release — 완료 |
...
- 웹과 MCP를 병행한다.
- 2D 설계에서는 DXF를 공식 제작 artifact로 사용한다. 제한형 Solid의 STEP approval은 local/CI에서만 제공하며 hosted Production capability가 아니다.
- 자연어보다 구조화 폼을 우선한다.
- 검증과 approval은 결정론적 코어만 수행한다.
- 로그인·DB·협업과 임의 형상의 범용 3D 생성·해석은 MVP에서 제외한다. 세 가지 제한형 solid family의 STEP 기하 검증은 지원한다.

## 19. CAD Artifact Assurance 확장 요구사항

| ID | 요구사항 | 우선순위 | 완료 조건 |
|---|---|---|---|
| DG-FR-019 | 외부 CAD artifact 감사 | P0 | DXF·STEP·IFC 원본 hash, unit, 구조·기하 metrics와 issue를 반환하고 `approval_eligible=false`를 유지한다. |
| DG-FR-020 | CAD revision compare | P0 | 같은 format 두 revision의 DXF geometry, STEP kernel metric 또는 IFC GlobalId 변화를 재현 가능하게 반환한다. |
| DG-FR-021 | 제한형 3D solid contract | P0 | mounting plate·angle bracket·flange의 정확한 mm 입력을 공개 schema로 검증한다. |
| DG-FR-022 | STEP 생성·독립 재입력 | P0 | writer와 분리된 OpenCascade process가 serialized STEP의 shape, bbox, topology와 cylindrical feature를 재측정하고 통과 시에만 bundle을 만든다. |
| DG-FR-023 | 실제 CAD 상호운용 evidence | P1 | 검증 STEP을 Rhino 8 headless document에서 B-rep으로 import하고 unit·bbox·3DM output evidence를 남긴다. |

DG-FR-021~023의 생성·검증·Rhino evidence는 local/CI에서 구현·검증되었다. 공개 Render Free
Production은 Solid 실행 capability를 의도적으로 비활성화하며, 이 상태는 기능 완료와 hosted 용량
검증을 구분하기 위한 운영 경계다.

3D solid의 `VERIFIED` 역시 계약된 형상과 저장 STEP의 일치만 뜻한다. 구조강도, 압력, 피로,
재료, 용접, 가공성과 산업표준 적합성은 판정하지 않는다.
