...
| 문서 버전 | 1.3.0 |
| 작성일 | 2026-07-12 |
| 제품 단계 | v0.2.0 공개 Production MVP |
| 설계·감사 범위 | 건축 평면 + 플랜트/반도체 utility piping + 기계/조선 plate + 제한형 3D solid + 외부 CAD audit |
| 기준 사용자 | 메이커·공학 학습자 |
...
DatumGuard는 공학설계 요구사항을 수치 기반 contract로 확정하고, CAD 산출물을 독립적으로 다시 읽어 치수·좌표·공차를 검증한 뒤 합격한 결과만 배포하는 Engineering Design Assurance Harness다. Architecture, Piping, Plate는 serialized DXF를 검증하고, 제한형 Solid은 local/CI에서 serialized STEP을 별도 OpenCascade process로 재입력한다. Artifact Lab은 contract 없는 외부 DXF·STEP·IFC를 변경하지 않고 감사하되 제작 승인을 만들지 않는다.
...
공개 demo의 기본 경로는 12×8m 합성 건축 평면으로 정확성 보증 흐름을 보여준다. 사용자는 wall, grid, opening, column과 room seed를 직접 이동하거나 mm 값으로 편집하고, 생성된 건축 DXF를 다시 읽은 검증 evidence가 통과한 뒤에만 공식 bundle을 받을 수 있다. `/piping`은 반도체 utility/플랜트 배관 route를, `/plate`는 기계·조선 plate/panel 설계를 같은 approval 원칙으로 제공한다. `/intake`는 외부 CAD informational audit를 제공한다. `/solid`은 세 가지 제한형 STEP family를 설명하지만 hosted 실행은 비활성이다.

### 1.1 v0.2.0 release 상태

- Release 기준점: [`v0.2.0`](https://github.com/tjwnsdhfz/datumguard/releases/tag/v0.2.0) / `40dd132ae8ca3267a79283f65058d6c8bbcee44b`
- Release CI: [256 pytest + 24 Playwright + container pass](https://github.com/tjwnsdhfz/datumguard/actions/runs/29181254550)
- Production smoke: [6 public route, API v0.2.0, Artifact Lab active, Solid disabled](https://github.com/tjwnsdhfz/datumguard/actions/runs/29181279413)
...
- v0.2.0의 256 pytest와 24 Playwright는 회귀·통합 test evidence이며, 위 100+50 benchmark를 완료한 것으로 계산하지 않는다.
...
| M8a | Docker와 v0.2.0 공개 release — 완료 |
