...
| 구성요소 | 기준 manifest | 책임 |
|---|---|---|
| Next.js web | `web/vercel.json`, `web/Dockerfile` | `/case-study`, `/`, `/piping`, `/plate`, `/solid`, `/intake`, `/privacy` UI와 API 호출 |
...
| Deployment smoke | `.github/workflows/deployment-smoke.yml` | Vercel 완료 뒤 호환성 검사, Render 완료 뒤 exact revision·web/API·CORS 검사 |
...
| FrameGuard — v0.3.0 screening | `$WEB_ORIGIN/frame` | `$API_ORIGIN/api/v1/frame/designs/run` |
...
| Privacy | `$WEB_ORIGIN/privacy` | 없음; data-boundary page |
...
Smoke checkout은 deployment SHA를 사용한다. Vercel event는 web sentinel과 backward-compatible API를 확인하고, Render event는 해당 `pyproject.toml` version, health `release_sha`, web route가 요구하는 API capability를 strict하게 확인한다. Preview는 base capability를 backward-compatible하게 확인한다. Architecture canary는 `architecture_four_room.json`, FrameGuard canary는 `frame_pipe_rack.json`, solid capability가 광고되면 solid canary는 `solid_mounting_plate.json`을 사용한다. Architecture/Solid은 official bundle과 핵심 geometry sentinel을 검사한다. FrameGuard는 deterministic solver ID, non-negative displacement, hash, violation 0과 screening disclaimer를 검사하며 구조 안전 승인을 주장하지 않는다. response payload는 log에 출력하지 않는다.

브라우저에서는 `/case-study`, `/`, `/piping`, `/frame`, `/plate`, `/solid`, `/intake`, `/openbim`, `/privacy`가 각각 유지되는지, 실패 preset에서 ZIP·DXF가 비활성인지, `passed` 응답과 검증 payload가 함께 있을 때만 다운로드가 활성인지 확인한다. 자동 deployment smoke는 `/case-study`와 `/frame`을 포함한 engineering workspace의 DOM sentinel을 검사하고, `/privacy` metadata와 local-data control은 Playwright에서 검사한다. CORS 오류가 나면 browser console의 요청 origin과 `DATUMGUARD_CORS_ORIGINS`를 exact string으로 비교한다.
...
| FrameGuard | `SCREENED PASS`, DXF 활성 | displacement 0.57mm / limit 0.65mm, utilization 0.042, DXF 0.001mm verified |
...
위 표의 FrameGuard 행은 v0.3.0 공개 UI/API에서 다시 확인했다. Vercel `/frame` sentinel, Render
`structural_frame` domain, deterministic canary, exact `release_sha`와 CORS는
[deployment smoke 29195107475](https://github.com/tjwnsdhfz/datumguard/actions/runs/29195107475)에서
같은 revision으로 통과했다. Render deployment는 `5413009032` / `dep-d99pkfeq1p3s73d3gjj0`,
Vercel production deployment는 `5413026696`이다.
...
- Browser: 실제 FastAPI와 Next.js를 함께 실행하는 7-workspace(FrameGuard·OpenBIM 포함) + Case Study +
  Privacy Playwright suite
...
- Optional Frame research: Python 3.12 base+dev+ml, genuine OpenSeesPy 3.8, focused Rhino/DXF/
  OpenSees/GNN tests와 작은 temporary training/inference parity smoke

Frame research workflow는 base Docker image에 Torch/OpenSeesPy를 포함하지 않으며 production
retraining도 수행하지 않는다. CI 성공은 원격 Vercel/Render 배포 성공이나 구조·안전·법규
적합성을 의미하지 않는다. 원격 인증, DNS, plan 선택과 실제 공개 URL 확인은 운영 단계다.
