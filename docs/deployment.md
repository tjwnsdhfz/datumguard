...
| FrameGuard — next release | `$WEB_ORIGIN/frame` | `$API_ORIGIN/api/v1/frame/designs/run` |
...
위 표는 현재 v0.2.1 production baseline이며 FrameGuard 배포 evidence가 아니다. FrameGuard는 PR merge,
Vercel `/frame` sentinel, Render `structural_frame` domain, deterministic canary, exact `release_sha`와
CORS가 같은 revision에서 통과한 뒤 별도 행을 추가한다.
...
- Browser: 실제 FastAPI와 Next.js를 함께 실행하는 6-workspace(FrameGuard 포함) + Case Study +
