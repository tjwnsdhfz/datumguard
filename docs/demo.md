...
|---|---|---:|
| `/` | Architecture default. Scenario guide, preset load, 안전한 mm 입력, drag/snap, 실제 독립 검증, 실패 복구, timeline/summary/download | 예 |
...
| `[data-testid="architecture-scenario"]` | `샘플 → 정확한 값 → 재측정` quick-start rail |
| `[data-testid="architecture-scenario-valid"]` | 정상 검증 시나리오 시작 |
| `[data-testid="architecture-scenario-failure"]` | 300mm 실패·복구 시나리오 시작 |
| `[data-testid="architecture-scenario-exact"]` | `column-a` 정확한 mm 편집 시작 |
...
| `[data-testid="architecture-run-verification"]` | quick-start rail의 상태 기반 실제 backend 검증·복구·download 주 행동 |
| `[data-testid^="architecture-inspector-"]` | Enter/blur 한 번에만 contract로 확정하는 mm 입력 |
| `[data-testid="architecture-mobile-object-select"]` | 900px 미만 화면의 객체 선택기 |
| `[data-testid="architecture-mobile-action"]` | 모바일 상태 기반 sticky 검증·복구·다운로드 행동 |
...
| `[data-testid="architecture-health"]` | 최대 70초 backend 준비 polling과 retry 상태 |
...
| `[data-testid="architecture-result-heading"]` | 검증 완료 뒤 자동 scroll/focus되는 판정 제목 |
| `[data-testid="architecture-repair-open-loop"]` | 실패 fixture의 `wall-north.end.x` 300→0mm 명시적 복구·재검증 |
| `a[href="/plate"]` | `플레이트 데모 열기` CTA |
| `[data-testid="plate-designer"]` | `/plate`의 기존 설계기 최상위 `<main>` |

검증 전 최상위 architecture node는 `data-verification-status="idle"`, 실행 중에는 `running`, 실제 응답이 통과한 뒤에만 `passed`를 가진다. Snap 완료 후 draggable에는 `data-snap-state="snapped"`가 설정된다. 검증 완료 후 결과 heading으로 focus를 이동하고 live region이 PASS 또는 차단 상태를 알린다. 각 stage는 색상만으로 구분하지 않고 heading과 설명 텍스트를 제공한다. `/plate`에는 Architecture로 돌아가는 링크가 있어야 한다.

수치 입력은 문자열 편집 중 contract를 변경하지 않는다. `Enter` 또는 blur에서 유한한 값을 한 번만 확정하고, `Escape`는 편집 전 값으로 돌아간다. 길이·두께·폭·깊이의 0 이하 값과 음수 offset은 inline 오류로 차단한다. 미확정 또는 잘못된 수치가 화면에 남아 있으면 검증 주 행동을 비활성화한다.
...
9. 실제 생성 동작이 없는 Wall·Column·Door·Window 도구는 활성 control로 노출하지 않는다.
10. 실패 violation의 관련 객체를 inspector로 연결하고, open-loop fixture의 복구 값은 `wall-north End X: 300 → 0 mm`로 명시한다.
11. 900px 미만에서는 drag를 비활성화하되 객체 선택·수치 입력·검증·ZIP 다운로드를 유지한다.
